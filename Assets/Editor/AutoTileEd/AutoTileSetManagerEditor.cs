...
public class AutoTileSetManagerEditor : Editor
{
	...
	void OnSceneGUI()
	{
		...
		if (currentEventType==EventType.Layout || currentEventType==EventType.ScrollWheel || current.button!=0)
		{
			skip=true;
		}
	...
	void ToggleTool()
	{
		if (pointHit && pointHit.gameObject.GetComponent<AutoTile>()!=null)
		{
			...
			{
			case AutoTileMode.Corner:
				newValue=AutoTileMode.Slope;
				break;
			case AutoTileMode.Slope:
				newValue=AutoTileMode.Corner;
				break;
			default:
				newValue=AutoTileMode.Corner;
				break;
