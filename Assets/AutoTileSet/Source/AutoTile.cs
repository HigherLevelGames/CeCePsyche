...
public class AutoTile : MonoBehaviour
{
	...
	//You can replace "GetNeighbours" with the method you want to use to find tile neighbours
		...
		for(i = -1; i < 2; i++)
		{
			for(j = -1; j < 2; j++)
			{
				if(!(i == 0 && j == 0))
				{
					try
					{
						target=Physics2D.OverlapCircle(new Vector2(x+i*size.x,y+j*size.y), 0.1f, autoTileLayer.value).gameObject;
						gates[index]=target!=null;
						if (gates[index])
						{
							newContacts.Add(target);
						}
					}
					catch
					{
